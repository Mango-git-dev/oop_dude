...
static bool isValidGio(const std::string& gio) {
    ...
    for (size_t i = 0; i < 5; ++i) {
        if (i == 2) continue;
        if (!std::isdigit(static_cast<unsigned char>(gio[i]))) return false;
    }
    try {
...
static bool isValidNgay(const std::string& ngay) {
    if (ngay.length() == 10 && ngay[4] == '-' && ngay[7] == '-') {
        // yyyy-mm-dd 
        for (size_t i = 0; i < 10; ++i) {
            if (i == 4 || i == 7) continue;
            if (!std::isdigit(static_cast<unsigned char>(ngay[i]))) return false;
        }
        return true;
    }
...
std::string ChuyenXeController::generateMaChuyen() const {
    ...
    for (const auto& entry : danhSachChuyenXe) {
        ...
        if (ma.rfind("CX", 0) == 0 && ma.length() > 2) {
            try {
                int idNum = std::stoi(ma.substr(2));
...
void ChuyenXeController::themChuyen() {
    ...
    std::string ngay;
    while (true) {
        ngay = view.getStringInput("Ngay khoi hanh (YYYY-MM-DD hoac DD/MM/YYYY): ");
        if (isValidNgay(ngay)) break;
        view.showError("Ngay khong hop le! Vui long nhap theo dinh dang YYYY-MM-DD hoac DD/MM/YYYY.");
    }
...
void ChuyenXeController::timKiemChuyen() {
    ...
    std::cout << "  1. Tim theo tu khoa (Ma chuyen, Noi di, Noi den, Bien so, Tai xe)\n";
    std::cout << "  2. Tim theo Noi di va Noi den tuong ung\n";
    std::cout << "  0. Quay lai\n";
    int chucNang = 0;
    while (true) {
        try {
    ...
    if (chucNang == 1) {
        std::string input = StringUtil::trim(view.getStringInput("Nhap tu khoa tim kiem (hoac '-1' de huy): "));
        if (input == "-1" || input.empty()) return;

        std::vector<ChuyenXe*> ketQua;
        for (const auto& entry : danhSachChuyenXe) {
            ChuyenXe* cx = entry.second;
            if (StringUtil::containsIgnoreCase(cx->getMaChuyen(), input) ||
                StringUtil::containsIgnoreCase(cx->getNoiDi(), input) ||
                StringUtil::containsIgnoreCase(cx->getNoiDen(), input) ||
                StringUtil::containsIgnoreCase(cx->getBienSoXe(), input) ||
                StringUtil::containsIgnoreCase(cx->getTenTaiXe(), input)) {
                ketQua.push_back(cx);
            }
        }

        if (ketQua.empty()) {
            view.showError("Khong tim thay chuyen xe nao phu hop voi tu khoa [" + input + "]!");
        } else {
            std::cout << "\nKet qua tim kiem (" << ketQua.size() << " chuyen):\n";
            view.displayDanhSachChuyenXe(ketQua);
        }
    } else {
        std::cout << "\n--- TIM KIEM CHUYEN XE TUONG UNG ---\n";
        std::string noiDi = StringUtil::trim(view.getStringInput("Nhap noi di (hoac de trong de qua): "));
        std::string noiDen = StringUtil::trim(view.getStringInput("Nhap noi den (hoac de trong de qua): "));

        if (noiDi.empty() && noiDen.empty()) return;

        std::vector<ChuyenXe*> ketQua;
        for (const auto& entry : danhSachChuyenXe) {
            ChuyenXe* cx = entry.second;
            bool matchDi = noiDi.empty() || StringUtil::containsIgnoreCase(cx->getNoiDi(), noiDi);
            bool matchDen = noiDen.empty() || StringUtil::containsIgnoreCase(cx->getNoiDen(), noiDen);
            if (matchDi && matchDen) {
                ketQua.push_back(cx);
            }
        }
        ...
        if (ketQua.empty()) {
            view.showError("Khong tim thay chuyen xe phu hop!");
        } else {
            std::cout << "\nKet qua tim kiem (" << ketQua.size() << " chuyen):\n";
            view.displayDanhSachChuyenXe(ketQua);
        }
...
void ChuyenXeController::suaThongTinChuyen() {
    ...
    std::string ngay;
    while (true) {
        ngay = view.getStringInput("Ngay khoi hanh moi (YYYY-MM-DD hoac DD/MM/YYYY): ");
        if (isValidNgay(ngay)) break;
        view.showError("Ngay khong hop le! Vui long nhap theo dinh dang YYYY-MM-DD hoac DD/MM/YYYY.");
    }
    ...
    if (cx->getType() == 2) {
        while (true) {
            try {
                phiVipMoi = view.getDoubleInput("Phu phi dich vu VIP moi (VND): ");
                if (phiVipMoi >= 0) break;
                view.showError("Phu phi khong the am! Vui long nhap lai.");
            } catch (const std::exception& e) { view.showError(e.what()); }
        }
    }
    ...
    if (view.confirmYN("Xac nhan sua thong tin?")) {
        ...
        if (cx->getType() == 2) {
            dynamic_cast<ChuyenXeVIP*>(cx)->setPhiDichVu(phiVipMoi);
        }
...
void ChuyenXeController::traCuuTaiXe() {
    ...
    std::string tenTX = StringUtil::trim(view.getStringInput("Nhap ten hoac tu khoa tai xe (hoac '-1' de huy): "));
    if (tenTX == "-1" || tenTX.empty()) return;

    std::vector<ChuyenXe*> filtered;
    for (const auto& entry : danhSachChuyenXe) {
        auto* cx = entry.second;
        if (StringUtil::containsIgnoreCase(cx->getTenTaiXe(), tenTX)) {
            filtered.push_back(cx);
        }
    }

    if (filtered.empty()) {
        view.showError("Khong tim thay tai xe nao phu hop voi tu khoa [" + tenTX + "]!");
    } else {
        std::cout << "\nCac chuyen xe cua tai xe phu hop tu khoa [" << tenTX << "]:\n";
...
void ChuyenXeController::xemLichTrinhTrongNgay() {
    ...
    std::string ngay = StringUtil::trim(view.getStringInput("Nhap ngay khoi hanh hoac tu khoa (YYYY-MM-DD hoac DD/MM/YYYY, '-1' de huy): "));
    if (ngay == "-1" || ngay.empty()) return;

    std::vector<ChuyenXe*> filtered;
    for (const auto& entry : danhSachChuyenXe) {
        auto* cx = entry.second;
        if (StringUtil::containsIgnoreCase(cx->getNgayKhoiHanh(), ngay)) {
            filtered.push_back(cx);
        }
    }

    if (filtered.empty()) {
        view.showError("Khong co chuyen xe nao khoi hanh voi tu khoa ngay [" + ngay + "]!");
    } else {
