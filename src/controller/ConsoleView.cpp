...
#include "../controller/StringUtil.h"
#include <iostream>
#include <limits>
#include <iomanip>
#include <cstdlib>
...
int ConsoleView::getIntInput(const std::string& prompt) const {
    ...
    if (!(std::cin >> value)) {
        ...
        throw InvalidDataException("Nhap sai dinh dang. Vui long nhap so nguyen.");
    }
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    return value;
}
...
double ConsoleView::getDoubleInput(const std::string& prompt) const {
    ...
    if (!(std::cin >> value)) {
        std::cin.clear(); 
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        throw InvalidDataException("Nhap sai dinh dang. Vui long nhap so.");
    }
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    return value;
}
...
void ConsoleView::displayVe(const Ve& ve) const {
    std::cout << "  Ma Ve: " << ve.getMaVe() 
              ...
              << " | Gia: " << StringUtil::formatCurrency(ve.getGiaTien()) << " VND"
              << " | Noi Di: " << ve.getNoiDi()
...
void ConsoleView::displayChuyenXe(const ChuyenXe* cx) const {
    if (cx) {
        std::cout << "  Ma: " << cx->getMaChuyen() 
                  ...
                  << " | DT: " << StringUtil::formatCurrency(cx->tinhDoanhThu()) << " VND\n";
    }
}
...
void ConsoleView::displayDanhSachChuyenXe(const std::map<std::string, ChuyenXe*>& ds) const {
    ...
    std::cout << "\n";
    std::cout << "+-----+------+--------+----------------+----------------+----------+------------+-------+------------------+------+------------------+\n";
    std::cout << "| STT | Loai | Ma     |    Noi Di      |    Noi Den     | KC (km)  |   Ngay     |  Gio  |    Tai Xe        |  HK  |   Doanh Thu      |\n";
    std::cout << "+-----+------+--------+----------------+----------------+----------+------------+-------+------------------+------+------------------+\n";
    int stt = 1;
    for (const auto& entry : ds) {
        ...
        std::cout << "| " << std::setw(3) << stt++ << " "
                  ...
                  << "| " << std::setw(16) << std::right << StringUtil::formatCurrency(cx->tinhDoanhThu()) << " |\n";
    }
    std::cout << "+-----+------+--------+----------------+----------------+----------+------------+-------+------------------+------+------------------+\n";
...
void ConsoleView::displayDanhSachVe(const std::vector<Ve>& ds) const {
    ...
    for (const auto& ve : ds) {
        std::cout << "| " << std::setw(3) << stt++ << " "
                  ...
                  << "| " << std::setw(12) << std::right << StringUtil::formatCurrency(ve.getGiaTien()) << " "
...
void ConsoleView::displayDanhSachChuyenXeChoKhach(const std::vector<ChuyenXe*>& ds, const std::vector<int>& choTrongList) const {
    ...
    for (size_t i = 0; i < ds.size(); ++i) {
        ...
        std::cout << "| " << std::setw(3) << (i + 1) << " "
                  ...
                  << "| " << std::setw(14) << std::right << StringUtil::formatCurrency(giaVe) << " VND |\n";
    }
    std::cout << "+-----+------+----------------+----------------+------------+-------+-----------+--------------------+\n";
