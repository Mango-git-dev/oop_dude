...
void VeController::datVeKhachHang() {
    ...
    if (availableTrips.empty()) {
        view.showError("Tat ca cac chuyen xe hien tai đều da day cho!");
        view.pressAnyKey();
        return;
    }

    std::cout << "DANH SACH CAC CHUYEN XE DANG CO SAN:\n";
    ...
    std::cout << "  Gia ve:          " << std::fixed << std::setprecision(0) << gia << " VND\n";
