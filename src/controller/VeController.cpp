...
std::string VeController::generateMaVe() const {
    ...
    for (const auto& ve : danhSachVe) {
        ...
        if (ma.rfind("VE", 0) == 0 && ma.length() > 2) {
            try {
...
void VeController::datVeKhachHang() {
    ...
    double gia = chuyenChon->tinhGiaVe();
...
void VeController::traCuuVeKhachHang() {
    ...
    std::string cccd;
    while (true) {
        cccd = StringUtil::trim(view.getStringInput("Nhap so CCCD hanh khach (12 chu so, hoac '-1' de huy): "));
        if (cccd == "-1" || cccd.empty()) return;
        if (isValidCCCD(cccd)) break;
        view.showError("So CCCD phai la 12 chu so! Vui long nhap lai.");
    }

    auto itHK = danhSachHanhKhach.find(cccd);
    ...
    if (itHK != danhSachHanhKhach.end()) {
        const auto& hk = itHK->second;
        std::cout << "\n-----------------------------------------------------\n";
        std::cout << "THONG TIN HANG KHACH:\n";
        std::cout << "  Ho ten: " << hk.getTen() << " | CCCD: " << hk.getCCCD() << " | SDT: " << hk.getSDT() << "\n";

        if (veOfHK.empty()) {
            std::cout << "\n[INFO] Khach hang [" << hk.getTen() << "] (CCCD: " << cccd << ") CHUA DAT chuyen xe nao.\n";
        } else {
            std::cout << "\nDanh sach ve xe da dat (" << veOfHK.size() << " ve):\n";
            view.displayDanhSachVe(veOfHK);
        }
    } else {
        if (veOfHK.empty()) {
            view.showError("Khong tim thay khach hang voi so CCCD [" + cccd + "] trong he thong!");
        } else {
            std::cout << "\nDanh sach ve xe da dat cua CCCD [" << cccd << "]:\n";
            view.displayDanhSachVe(veOfHK);
        }
    }
    view.pressAnyKey();
...
void VeController::huyVeKhachHang() {
    ...
    std::string cccd;
    while (true) {
        cccd = StringUtil::trim(view.getStringInput("Nhap so CCCD hanh khach (12 chu so, hoac '0' de huy): "));
...
void VeController::timKiemVe() {
    view.clearScreen();
    std::cout << "TIM KIEM VE XE\n\n";

    std::string input = StringUtil::trim(view.getStringInput("Nhap tu khoa tim kiem (Ma ve, Ma chuyen, CCCD, Noi di, Noi den, hoac '-1' de huy): "));
    if (input == "-1" || input.empty()) return;

    std::vector<Ve> ketQua;
    for (const auto& ve : danhSachVe) {
        if (StringUtil::containsIgnoreCase(ve.getMaVe(), input) ||
            StringUtil::containsIgnoreCase(ve.getMaChuyenXe(), input) ||
            StringUtil::containsIgnoreCase(ve.getCCCDHanhKhach(), input) ||
            StringUtil::containsIgnoreCase(ve.getNoiDi(), input) ||
            StringUtil::containsIgnoreCase(ve.getNoiDen(), input)) {
            ketQua.push_back(ve);
        }
    }

    if (ketQua.empty()) {
        view.showError("Khong tim thay ve xe nao phu hop voi tu khoa [" + input + "]!");
    } else {
        std::cout << "\nKet qua tim kiem (" << ketQua.size() << " ve):\n";
        view.displayDanhSachVe(ketQua);
    }
    view.pressAnyKey();
}
...
void VeController::suaThongTinVe() {
    ...
    std::string maChuyen;
    while (true) {
        maChuyen = view.getStringInput("Nhap ma chuyen xe moi (hoac '-1' de huy): ");
        if (maChuyen == "-1" || maChuyen.empty()) return;
        if (danhSachChuyenXe.count(maChuyen)) break;
        view.showError("Chuyen xe [" + maChuyen + "] khong ton tai trong he thong! Vui long nhap lai.");
    }
...
void VeController::traCuuKhachHang() {
    ...
    std::string input = StringUtil::trim(view.getStringInput("Nhap ten hoac CCCD khach hang (hoac '-1' de huy): "));
    if (input == "-1" || input.empty()) return;

    std::vector<HanhKhach> ketQuaHK;
    for (const auto& entry : danhSachHanhKhach) {
        const auto& hk = entry.second;
        if (StringUtil::containsIgnoreCase(hk.getTen(), input) ||
            StringUtil::containsIgnoreCase(hk.getCCCD(), input)) {
            ketQuaHK.push_back(hk);
        }
    }

    if (ketQuaHK.empty()) {
        view.showError("Khong tim thay khach hang nao phu hop voi tu khoa [" + input + "]!");
    } else {
        std::cout << "\nFound " << ketQuaHK.size() << " khach hang phu hop voi tu khoa [" << input << "]:\n";
        for (size_t i = 0; i < ketQuaHK.size(); ++i) {
            const auto& hk = ketQuaHK[i];
            std::cout << "\n-----------------------------------------------------\n";
            std::cout << "KHACH HANG " << (i + 1) << "/" << ketQuaHK.size() << ":\n";
            view.displayHanhKhach(hk);

            std::vector<Ve> veOfHK;
            for (const auto& ve : danhSachVe) {
                if (StringUtil::trim(ve.getCCCDHanhKhach()) == StringUtil::trim(hk.getCCCD())) {
                    veOfHK.push_back(ve);
                }
            }
